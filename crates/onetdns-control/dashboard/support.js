...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    // document-start 사용자 스크립트가 <x-dc>의 미해결 style/ref/event 속성을
    // 건드릴 수 있다. live DOM은 빠른 첫 화면에만 사용하고, 서버에서 다시 받은
    // 원문 HTML로 템플릿을 즉시 교체해 {{ ... }} 바인딩을 보존한다.
    fetch(location.href, { cache: "no-store", credentials: "same-origin" })
      .then((res) => res.ok ? res.text() : "")
      .then((text) => {
        const raw = text ? parseDcText(text) : null;
  ...
  function collectProps(node, isComponent, host) {
    ...
    for (const { name, value } of [...node.attributes]) {
      ...
      let key = name;
      if (key.startsWith("data-dc-bind-")) key = key.slice(13);
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      __setLogicState(update, cb) {
        ...
        if (this.__prevLogicState === void 0) this.__prevLogicState = prev;
      ...
      componentDidUpdate(prevProps) {
        ...
        if (this.__needsDidMount) {
          ...
          try {
            const prevState = this.__prevLogicState;
            this.__prevLogicState = void 0;
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      return Promise.reject(new Error("JSX imports require a locally bundled Babel runtime"));
    }
